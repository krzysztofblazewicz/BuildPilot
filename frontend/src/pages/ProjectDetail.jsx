...
const DIFF_COLORS = {
  Easy: "border-white/15 bg-white/[0.04] text-zinc-200",
  Medium: "border-white/20 bg-white/[0.08] text-zinc-100",
  Hard: "border-zinc-200 bg-zinc-200 text-black",
};

const SEVERITY_COLORS = {
  High: "border-white/25 bg-white/[0.08] text-white",
  Medium: "border-white/15 bg-white/[0.04] text-zinc-200",
  Low: "border-white/10 bg-transparent text-zinc-400",
};
...
const STAGE_PILL_STYLES = {
  Idea: "border-white/10 text-zinc-400",
  Validating: "border-white/25 text-zinc-200",
  Planning: "border-white/15 bg-white/[0.06] text-zinc-200",
  "Building MVP": "border-white/25 bg-white/[0.12] text-white",
  Testing: "border-white/10 bg-white/[0.03] text-zinc-300",
  "Ready to Launch": "border-zinc-300 bg-zinc-300 text-black",
  Launched: "border-white bg-white text-black",
};
...
export default function ProjectDetail() {
  ...
  const toggleChecklistItem = async (itemId) => {
    ...
    setProject({ ...project, checklist: next });
    try {
    ...
    return (
      <div className="min-h-screen bg-[#050505] text-white">
        ...
        <main className="mx-auto max-w-3xl px-6 py-20 text-center">
          <h1 className="font-heading text-2xl text-white">{error}</h1>
          <Link
            to="/dashboard"
            data-testid="detail-back-error"
            className="mt-4 inline-flex items-center gap-2 text-zinc-200 hover:text-white"
          >
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
            ...
            <div ref={exportRef} className="relative">
              ...
              {exportOpen && (
                <div
                  ...
                  className="absolute right-0 z-30 mt-1 w-60 overflow-hidden rounded-md border border-white/10 bg-[#0d0d0d] shadow-2xl shadow-black/50"
                >
                  {EXPORT_VARIANTS.map((v) => (
                    <button
                      ...
                      className="block w-full px-3 py-2 text-left text-xs text-zinc-200 hover:bg-white/[0.04]"
                    >
                      {v.label}
                    </button>
                  ))}
        ...
        <header className="mt-6">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-white/10 bg-white/[0.02] px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-widest text-zinc-300">
              {project.goal_type}
            </span>
            <span
              className={`rounded-full border px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-widest ${
                project.status === "Refined"
                  ...
                  ? "border-white/20 bg-white/[0.04] text-zinc-200"
            ...
            {/* Stage selector */}
            <span
              className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-0.5 text-[10px] font-mono uppercase tracking-widest ${
                STAGE_PILL_STYLES[project.stage] || STAGE_PILL_STYLES.Idea
              }`}
            >
              Stage
              <select
          ...
          {editing ? (
            <input
              ...
              className="input-base mt-3 text-2xl font-heading font-bold tracking-tight"
            />
          ) : (
            <h1 className="mt-3 font-heading text-3xl font-bold tracking-tight sm:text-4xl text-white">
              {project.title}
            </h1>
          )}
          {plan?.pitch && !editing && (
        ...
        <section data-testid="checklist-panel" className="mt-8 surface p-6">
          <header className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-white/10 bg-white/[0.03]">
                <ListChecks className="h-3.5 w-3.5 text-zinc-300" />
              </span>
              <h3 className="font-heading text-base font-semibold tracking-tight text-white">
                Validation Checklist
              </h3>
            </div>
            <span data-testid="checklist-progress" className="font-mono text-xs text-zinc-400">
                ...
                <button
                  ...
                  className={`group flex w-full items-center gap-3 rounded-md border px-3 py-2 text-left text-sm transition-colors ${
                    it.done
                      ...
                      : "border-white/10 bg-[#101010] text-zinc-300 hover:border-white/20 hover:bg-white/[0.03]"
                  }`}
                >
                  {it.done ? (
                    <CheckCircle2 className="h-4 w-4 shrink-0 text-zinc-200" />
                  ) : (
                    <Circle className="h-4 w-4 shrink-0 text-zinc-500 group-hover:text-zinc-300" />
                  )}
                  <span className={it.done ? "line-through opacity-70" : ""}>{it.label}</span>
                </button>
              </li>
            ))}
          </ul>
        ...
        {/* AI mode bar */}
        <section data-testid="ai-modes-bar" className="mt-6 surface p-6">
              ...
              return (
                <button
                  ...
                  className={`group flex flex-col items-start gap-2 rounded-md border px-3 py-3 text-left text-xs transition-all disabled:opacity-70 ${
                      ...
                      : "border-white/10 bg-[#101010] text-zinc-200 hover:border-white/20 hover:bg-white/[0.04]"
                  }`}
                >
        ...
        {!plan ? (
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-12">
            ...
            <Card title="Risks & Challenges" icon={ShieldAlert} className="md:col-span-5">
              <List items={plan.risks} accent="muted" />
            </Card>

            <Card title="4-Week Build Roadmap" icon={MapIcon} className="md:col-span-12">
              <div className="grid gap-3 md:grid-cols-4">
                {(plan.roadmap || []).map((wk) => (
                  <div key={wk.week} className="rounded-md border border-white/10 bg-[#101010] p-4">
                    <div className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">
                      Week {wk.week}
                    </div>
                    ...
                    <ul className="mt-3 space-y-2 text-xs text-zinc-300">
                      {(wk.tasks || []).map((t, i) => (
                        <li key={i} className="flex items-start gap-2">
                          <span className="mt-1.5 h-1 w-1 rounded-full bg-zinc-300" />
                          {t}
                        </li>
                      ))}
                    </ul>
            ...
            <Card title="Ethics & Privacy" icon={ScrollText} className="md:col-span-6">
              <List items={plan.ethics} accent="muted" />
            </Card>
            <Card title="Your Next 3 Actions" icon={Gauge} className="md:col-span-6">
              <ol className="space-y-2 text-sm text-zinc-200">
                {(plan.next_actions || []).map((t, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-3 rounded-md border border-white/15 bg-white/[0.03] p-3"
                  >
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-white/25 bg-white/[0.06] text-[10px] font-bold text-white">
                      {i + 1}
                    </span>
                    {t}
                  </li>
    ...
    >
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="font-heading text-sm font-semibold tracking-tight text-white">{label}</div>
          {result.headline && (
            <span className="text-xs text-zinc-400">— {result.headline}</span>
          )}
        </div>
        <button
          data-testid={`ai-mode-${modeKey}-close`}
          onClick={onClose}
          className="rounded-md p-1 text-zinc-500 hover:bg-white/[0.04] hover:text-zinc-100"
      ...
      {result.error ? (
        <p className="text-sm text-zinc-300">{String(result.error)}</p>
      ) : (
...
function RenderModeBody({ mode, result }) {
    ...
    return (
      <div className="space-y-3 text-sm text-zinc-200">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Bullets title="Improvements" items={result.improvements} accent="silver" />
          <Bullets title="Ask yourself" items={result.ask_yourself} accent="muted" />
        </div>
        {typeof result.confidence === "number" && (
          <div className="font-mono text-xs text-zinc-500">Confidence: {result.confidence}%</div>
        )}
      </div>
    );
  }
  if (mode === "build-mvp") {
    return (
      <div className="grid gap-3 text-sm text-zinc-200 md:grid-cols-3">
        <Bullets title="Must have" items={result.must_have} accent="silver" />
        <Bullets title="Explicitly cut" items={result.explicitly_cut} accent="muted" />
        <Bullets title="Definition of done" items={result.definition_of_done} accent="silver" />
      ...
      <div className="space-y-3 text-sm text-zinc-200">
        <div className="grid gap-2 sm:grid-cols-2">
          {Object.entries(result.recommended || {}).map(([k, v]) => (
            <div key={k} className="rounded-md border border-white/10 bg-[#101010] p-2.5">
              <div className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">
                {k}
              </div>
              <div className="mt-1 text-sm text-zinc-100">{v}</div>
        ...
        <Bullets title="Tradeoffs" items={result.tradeoffs} accent="muted" />
      </div>
    );
  }
  if (mode === "check-risks") {
    return (
      <div className="space-y-2 text-sm text-zinc-200">
        {(result.risks || []).map((r, i) => (
          <div key={i} className="rounded-md border border-white/10 bg-[#101010] p-3">
  ...
  if (mode === "launch-plan") {
    return (
      <div className="grid gap-3 text-sm text-zinc-200 md:grid-cols-3">
        <Bullets title="Pre-launch" items={result.pre_launch} accent="silver" />
        <Bullets title="Launch day" items={result.launch_day} accent="silver" />
        <Bullets title="Post-launch" items={result.post_launch} accent="muted" />
  ...
  if (mode === "generate-pitch") {
    return (
      <div className="space-y-3 text-sm text-zinc-200">
        ...
        <Bullets title="60-second demo script" items={result.demo_script} accent="silver" />
      </div>
    );
  }
  return <pre className="text-xs text-zinc-500">{JSON.stringify(result, null, 2)}</pre>;
}
...
function PitchBlock({ label, text }) {
  return (
    <div className="rounded-md border border-white/10 bg-[#101010] p-3">
      ...
      <p className="mt-1 text-sm text-zinc-100">{text}</p>
    </div>
  );
}

function Bullets({ title, items, accent = "silver" }) {
  const dotColor = accent === "muted" ? "bg-zinc-500" : "bg-zinc-200";
  return (
    <div>
...
function List({ items, accent = "silver" }) {
  const dot = accent === "muted" ? "bg-zinc-500" : "bg-zinc-200";
  return (
...
function Score({ score, className = "" }) {
  ...
  return (
    <section
      ...
      className={`relative overflow-hidden surface p-6 ${className}`}
    >
...
function NoPlan({ onGenerate, loading }) {
  return (
    <div className="mt-10 rounded-xl border border-dashed border-white/10 bg-[#0a0a0a] p-12 text-center">
      <Sparkles className="mx-auto h-6 w-6 text-zinc-300" />
      <h3 className="mt-3 font-heading text-xl font-semibold text-white">No plan generated yet</h3>
      ...
      <button
        ...
        onClick={onGenerate}
        disabled={loading}
        className="btn-primary mt-5 inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium"
      >
...
function buildLaunchChecklist(project) {
  ...
  if (p) {
    ...
    (p.next_actions || []).forEach((t) => lines.push(`- [ ] ${t}`));
    lines.push("");
